...
import com.example.study_app.DiscussionActivity;
    ...
    private void showAddAnswerDialog(Discussion discussion) {
        try {
            ...
            builder.setPositiveButton("Gửi trả lời", (dialog, which) -> {
                ...
                if (!content.isEmpty()) {
                    // Gọi API để tạo answer
                    if (context instanceof DiscussionActivity) {
                        DiscussionActivity activity = (DiscussionActivity) context;
                        activity.createAnswer(discussion.getId(), content);
                    }
                } else {
    ...
    private void showDeleteConfirmDialog(int questionId) {
        try {
            ...
            builder.setPositiveButton("Xóa", (dialog, which) -> {
                if (context instanceof DiscussionActivity) {
                    DiscussionActivity activity = (DiscussionActivity) context;
                    activity.deleteQuestion(questionId);
                }
            });
